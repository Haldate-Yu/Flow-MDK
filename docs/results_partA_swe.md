...
† 6 层 GCN 塌缩为常数型输出（逐场景 RMSE 31.7–36.2 m、跨域 24.7–127 m，
best_epoch=14/113）——深传播无门控在链式图上不可用，与 3 层 GCN 0.79 形成对照。
...
1. **主干稳定性判据通过（单种子抽样）**：flow_mdk 论文预算下无本地 s1 型崩坏
   （A-test 1.42、B1 0.875）；崩坏转移到无门控的深基线上（GCN 塌缩、GAT 退化）。
   「ψ 差值消息主干不稳定」在论文预算下未复现，但需 3 种子确认。
2. **合成 1D 域内与多数零样本域：浅层 8 跳 SWE-GNN 最优**（0.72 vs 1.42）；
   6 层深度+MDK 未换来域内收益。fam_mdx（最难闸门族）零样本 flow_mdk 最优
   （2.28 vs 3.02），与 L2 单种子方向一致但本地多种子未复现，权重存疑。
3. **真实家族域（B1，论文核心域）：深层门控系全面占优**（ssgc/flow_mdk
   0.85/0.87 vs swegnn 2.15，KS p≈7e-4）；此处深度+门控是收益方向。
4. **定向（flow_mdk）vs 对称（ssgc）仍无显著差异**：B1 域内 p=0.81、B2 p=1.0；
   逐考卷 2:2 分胜负（flow_mdk 拿 mdx/mdx_upstream，ssgc 拿 mdx_downstream/wqh）。
   本地预算唯一持久的定向信号（mdx_upstream）在论文预算下仍偏向 flow_mdk
   （1.33 vs 1.78）但不显著。**「水力定向 MDK 优于对称核」的核心假设至今
   （本地 3 种子 + 论文单种子）无统计支持。**
5. **共性异常：best epoch 极早（4–14）+ val 漂移 2–4×**（如 partA flow_mdk
   5.38@13 → 45.6@112），所有 7 个 paper run 一致；λ₀ 末层持续收缩
   （0.4→0.05，曲线 `runs/lambda0_curves_paper.png`）。评测协议用 last.pt，
   best.pt 对照评估与 Dirichlet 富评待做（checkpoint 已取回，本地可补）。
