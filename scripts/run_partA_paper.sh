...
#
# Results -> runs/partA_paper_<name>/ (kept separate from the first-round
# runs/partA_<name>/ so both archives survive the runs/ pull-back).
#
# Server usage (A100 GPU1):
#   CUDA_VISIBLE_DEVICES=1 bash scripts/run_partA_paper.sh 2>&1 \
#       | tee data/partA_paper_experiments.log
set -e
cd "$(dirname "$0")/.."
...
train_and_eval () {
    ...
    echo "=== [$name] training (paper settings from $config) ==="
    python scripts/train.py --config "$config" --set \
        out_dir=runs/partA_paper_$name $COMMON 2>&1 | tail -3
    echo "=== [$name] Part A test (A1+A2+A3, 50 scenarios) ==="
    python scripts/evaluate.py --config runs/partA_paper_$name/config.yaml \
        --checkpoint runs/partA_paper_$name/last.pt --split test 2>&1 | tail -2
    echo "=== [$name] Part A2 test (unseen hydrology) ==="
    python scripts/evaluate.py --config runs/partA_paper_$name/config.yaml \
        --checkpoint runs/partA_paper_$name/last.pt --split A2_test 2>&1 | tail -2
    echo "=== [$name] Part A3 test (larger basins) ==="
    python scripts/evaluate.py --config runs/partA_paper_$name/config.yaml \
        --checkpoint runs/partA_paper_$name/last.pt --split A3_test 2>&1 | tail -2
    echo "=== [$name] zero-shot family_mdx ==="
    python scripts/evaluate.py --config runs/partA_paper_$name/config.yaml \
        --checkpoint runs/partA_paper_$name/last.pt --split test \
    ...
    python scripts/evaluate.py --config runs/partA_paper_$name/config.yaml \
        --checkpoint runs/partA_paper_$name/last.pt --split val \
