...
- mass-balance relative errors parsed from the solver listing (the projects
  run with MASS-BALANCE = True; machine-precision volume errors are the
  strongest internal consistency signal available);
...
COURANT_RE = re.compile(r"MAXIMUM COURANT NUMBER:\s*([0-9.E+-]+)")


def parse_listing(listing: Path | None) -> dict:
    """Volume-balance errors + Courant numbers from the solver listing."""
    text = (listing.read_text(encoding="utf-8", errors="replace")
            if listing and listing.exists() else "")
    vol = [abs(float(v)) for v in VOLUME_RE.findall(text)]
    courant = [float(v) for v in COURANT_RE.findall(text)]
    return {
...
def ingest_results(results: Path, mesh: Path, listing: Path | None = None,
                   solver_tag: str = "telemac2d_docker_rerun",
                   force: bool = False) -> dict:
    """Attach a results SELAFIN as truth onto a 2D mesh/grid scenario npz.

    Works for both layouts: real-project dual-graph meshes (``2d_mesh``) and
    the synthetic structured grids (``2d_grid``); both carry dynamic channels
    ["h", "|q|"]. Returns the QC dict (also stored in scenario.meta).
    """
    scenario = load_scenario(mesh)
    layout = scenario.meta.get("layout")
    assert layout in ("2d_mesh", "2d_grid"), f"unexpected layout {layout}"
    if scenario.meta.get("solver") == solver_tag and not force:
        raise SystemExit("truth already attached; use --force to overwrite")

    res = Selafin(results)
    if res.npoin != scenario.node_static.shape[0]:
        raise SystemExit(
            f"mesh mismatch: results npoin={res.npoin} vs npz "
            f"{scenario.node_static.shape[0]}")
    ...
    scenario.meta["solver"] = solver_tag
    scenario.meta["telemac2d_truth"] = {**qc, "problems": problems,
                                        "results": str(results)}
    save_scenario(scenario, mesh)
    return {**qc, "problems": problems}
...
def main() -> None:
    ...
    results = args.workdir / args.results
    listing = args.listing or (args.workdir.parent / f"{args.workdir.name}_run.log")
    qc = ingest_results(results, args.mesh, listing, force=args.force)

    err = qc["volume_rel_err_max"]
    print(f"[2d] {args.mesh.stem}: {qc['frames']} frames @ {qc['cadence_s']:.0f}s, "
