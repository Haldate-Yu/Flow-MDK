...
\item \_post: this were the post-treatment is done. We recommand to use
...
\item For a basic validation case (steering file run in sequential and parallel
  ...
  sequential vs parallle) as well as a couple examples of post-treatment have a
...
\begin{enumerate}
  ...
  \item Launch all the \telma runs via the job scheduler
...
following points:
\begin{itemize}
  \item An HPC configuration (chech the website for more information).
...
The procedure below will submit each of the \telma{} studies to the cluster
scheduler. This means that in the best of case your whole validation will be as
long as your longest \telma{} run.
The "commands" with the argument hpc=True will be submetted as well.
...
\begin{enumerate}
  ...
  \item Run '\verb!validate\_telemac.py! \textit{valid\_options}
...
\begin{itemize}
  \item \textit{valid\_options} by the options for your validation (--tag, -k,
    --bypass\ldots).
  \item \textit{hpc\_options} by the options you would give an HPC \telma{} run
    on your cluster (--queue, --nctile, --walltime, --jobname,
    --project\ldots).
\end{itemize}
