...
There are two models available in \khione to describe frazil ice evolution, the mono-class model
and the multi-class model, which rely on different radius discretization as shown in Figure \ref{fig:radius_discretization}.

\begin{figure}[H]
    \begin{center}
        \includegraphicsmaybe{[width=0.78\textwidth]}{./graphics/monoclass_vs_multiclass.png}
    \end{center}
    \caption{Typical log-normal distribution of frazil crystals and mono-class (left) versus multi-class (right) radius discretization.}
    \label{fig:radius_discretization}
\end{figure}

In the case of the mono-class model, all particles are assumed to have the same radius, noted $r$.
...
On the other hand, the multi-class model consists in considering several classes, each one associated with a radius, noted $r_k$ for the $k^{\rm{th}}$ class.
...
\telkey{NUMBER OF CLASSES FOR SUSPENDED FRAZIL ICE} (default = 1). If only one class is selected, the mono-class model is used,
otherwise it is the multi-class model.

\paragraph{Mono-class model}
The mono-class model relies on one tracer to model frazil ice volume fraction.
...
\paragraph{Multi-class model}
As opposed to the mono-class model, the multi-class model relies on
...
\begin{itemize}
	...
	\item $S^k_{SN}$ is the secondary nucleation source term described in section \ref{section:secondary_nucleation}, which can be activated using the keyword \telkey{MODEL FOR THE SECONDARY NUCLEATION} (default = 1),
...
\begin{WarningBlock}{Note:}
    The flocculation and secondary nucleation source terms are only computed in the multi-class model
...
Frazil crystals are supposed to have the same disk shaped geometry, as described in Figure \ref{fig:crystal_shape}, characterized by a radius $r_k$ and a thickness $e_k$, related with a constant ratio $R$ such that $e_k = 2r_k/R$.
The radius is defined via the keyword \telkey{FRAZIL CRYSTALS RADIUS} (default = 1.2E-4).
...
depends on the salinity $S$ such that $T_f(S) = -0.0575S + 0.00171S^{3/2} - 0.00021S^2$ if \telkey{SALINITY=YES},
    \item $l_k$ is the characteristic length scale of the $k^{\rm{th}}$ class, supposed to be equal to $r_k$ as suggested in \cite{holland_JFM_2005, holland_2006},
...
\begin{figure}[H]
    ...
    \end{center}
    \caption{Multi-class model mass exchanges caused by thermal growth or melting}
    \label{fig:sgm_class_jumps}
\end{figure}

As explained in \cite{daly_1994}, frazil crystals are supposed to
grow only from their edges because of their disk shape, which
leads to the production rate for thermal growth defined by:
\begin{equation}
G_{k}=\frac{K_{w}Nu_{k}}{L_{i}\rho_{i}}(T_{f}-T)\frac{2}{r_{k}^{2}}C_{k},
\label{eq:model:sgm:source_growth}
\end{equation}
whereas the melting is supposed to occur on all surfaces of the
disk which leads to:
\begin{equation}
M_{k}=\frac{K_{w}Nu_{k}}{L_{i}\rho_{i}}(T_{f}-T)\frac{2}{r_{k}}\left(\frac{1}{r_{k}}+\frac{1}{e_{k}}\right)C_{k},
...
For the mono-class frazil ice model (when only one class of frazil is selected),
...
As explained in Figure \ref{fig:sgm_monoclass} the mono-class thermal growth source term
...
considered in the case of the mono-class model.

\begin{figure}[H]
    \begin{center}
        \includegraphicsmaybe{[width=0.45\textwidth]}{./graphics/sgm_monoclass.png}
    \end{center}
    \caption{Mono-class model thermal growth principle}
...
which can be selected with the keyword \telkey{MODEL FOR THE SECONDARY NUCLEATION} (default = 1):
\begin{lstlisting}
...
can be tuned using the \telkey{SECONDARY NUCLEATION NMAX PARAMETER} (default = 1.E3).
...
The model can be selected via the keyword \telkey{MODEL FOR THE BUOYANCY VELOCITY} (default = 1):
\begin{lstlisting}
1 : DALY (1984)
...
\end{itemize}
\item The previous formulation was later simplified into $w_k=30r_k^{1.2}$ which only
takes into account the intermediate range (units for radius and rise velocity in this formulation
are cm and cm/s).
...
All formulations are plotted on Figure \ref{fig:rise_velocity} with $R=8$ and $C_d=2$.
\begin{figure}[H]
  \centering
  \includegraphics[width=0.65\textwidth]{graphics/figure_rise_velocity.pdf}
  \caption{Rise velocity formulations with $R=8.$ (in Daly 1984), $C_d=2$ (in Gosink \& Osterkamp 1983)}
...
\telkey{MINIMUM NUMBER OF FRAZIL CRYSTALS} (default = 1.5E4). If multiple classes are selected,
the number of primary nuclei is equally shared between all classes.
...
\phi_s = (1-C_i) \phi_{aw} + C_i \phi_{iw},
\end{equation}
in which
\begin{itemize}
...
Therefore Equation \eqref{eq:model:temperature_water_phase} can be developed as:
\begin{equation}
...
the turbulent viscosity $\nu_{t}$ and the turbulent intensity $\alpha_{T}$ need 
to be provided with the keyword
\telkey{CONSTANT TURBULENCE PARAMETERS}
 (default: $9.6 \times 10^{-4}$;$12 \times 10^{-4}$;$6.9 \times 10^{-5}$;$8.7 \times 10^{-2}$).
