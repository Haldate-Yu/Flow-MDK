...
\left\{
    \begin{array}{l}  
  (\sum^{N_c}_{k=1} S_{MI}^k)/ t_{imin} \text{ if } C_i < 1 \\
  0 \text{ else}
    \end{array}
    \right. ,
\end{equation}
...
\begin{equation}
  S_{M}^t=\left\{
    \begin{array}{l}
      \sum^{N_c}_{k=1} S_{MI}^k  \text{ if } C_i \geq 1 \\
      0 \text{ else}
    \end{array}
    \right..
\end{equation}
...
\begin{equation}
\Delta V_i = \sum_{k=1}^{N_c} \left( D_k w_k C_k - E_k C_i t_i \right) \Delta t \Delta S 
\end{equation}
in which,
\begin{itemize}
  \item $N_c$ is the number of frazil class,
