...
\item Fortran subroutine {\ttfamily bedload\_engel\_cc\_gaia.f}.
\end{itemize}
...
\item Fortran subroutine {\ttfamily bedload\_wilcock\_crowe\_gaia.f}.
\end{itemize}
...
roughness through the associated keywords \telkey{COMPUTE BED ROUGHNESS AT SEDIMENT SCALE} (logical type variable, set to {\ttfamily = NO} by default) and \telkey{BED ROUGHNESS PREDICTOR OPTION}. It is recalled that the bed friction option of \gaia is not
