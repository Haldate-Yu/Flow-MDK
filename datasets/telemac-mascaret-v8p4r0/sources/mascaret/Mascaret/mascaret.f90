...
!
! MODIFIED BY MAURICE FOR READ Q_XAJ
module READ_XAJ_FILES
   implicit none
   contains
   !
   SUBROUTINE READ_Q_XAJ(NLINES,Q_XAJ)
      implicit none
      integer :: i,iostat,time,num_columns,num_lines,iostat_value,J,N
      integer,intent(out) :: nlines
      real(kind=8),allocatable,intent(  out) :: Q_XAJ(:,:)
      character(len=256)  :: line
      character(len=1),dimension(:),allocatable :: char_line
      !
      PRINT*,'READING XAJ FILES'
      open(unit=183,file='Q_XAJ.txt',status='old',action='read',iostat=iostat)
      if (iostat == 0) then
         PRINT*,'TEST 0'
         read(183,'(A)',iostat=N) line ! read first line
         if (N /= 0) then
            print*,'NO CONTENT IN XAJ FILE'
         endif
         ! calculate lines
         num_lines = 0
         do 
         ...
         allocate(char_line(len_trim(line)))
         do i=1, len_trim(line)
            char_line(i) = line(i:i)
         enddo
         ! calculate columns
         num_columns = 1
         do i=1, size(char_line)
            if(char_line(i) == ' ') then
               num_columns = num_columns +1
            endif
         enddo
         print*,'columns',num_columns
         allocate(Q_XAJ(num_lines,num_columns))
         read(183,'(A)',iostat=iostat_value) LINE
         DO WHILE(.TRUE.)
            read(183,'(A)',iostat=iostat_value) LINE
            IF (iostat_value /= 0) EXIT
            NLINES = NLINES + 1
            READ(LINE,*) (Q_XAJ(NLINES,J),J=1,num_columns)
         END DO
         close(183)
      else
         allocate(Q_XAJ(1,1))
         Q_XAJ = 0.D0
         PRINT*,'NO XAJ FILES TO READ'
      end if
   END SUBROUTINE READ_Q_XAJ
   !
   SUBROUTINE READ_X_XAJ(num_columns,X_XAJ)
      implicit none
      integer :: i,iostat,time,num_lines,iostat_value,J,N
      integer,intent(out) :: num_columns
      real(kind=8),allocatable,intent(  out) :: X_XAJ(:)
      character(len=256)  :: line
      character(len=1),dimension(:),allocatable :: char_line
      open(unit=185,file='X_XAJ.txt',status='old',action='read',iostat=iostat)
      IF (iostat==0) THEN
         read(185,'(A)',iostat=N) line
         allocate(char_line(len_trim(line)))
         do i=1, len_trim(line)
            char_line(i) = line(i:i)
         enddo
         ...
         enddo
         print*,'columns OF X',num_columns
         allocate(X_XAJ(num_columns))
         DO WHILE(.TRUE.)
            read(185,'(A)',iostat=iostat_value) LINE
            IF (iostat_value /= 0) EXIT
            READ(LINE,*) (X_XAJ(J),J=1,num_columns)
         END DO
         close(185)
      ELSE
         ALLOCATE(X_XAJ(1))
         ...
         PRINT*,'NO XAJ FILES TO READ'
      ENDIF
   end subroutine READ_X_XAJ
end module READ_XAJ_FILES
...
subroutine MASCARET( &
                     ...
                     FRTIMP ,                    & ! Indicateur pour l'implicitation du frottement
...
!-----------------------------------------------------------------------
   ...
   integer KHYLIM
   integer                                 :: NBARAD,INOEUD,icompt
   integer I,IDEB(20),IFIN(20),INDIC(20)
   integer INDCO(20),ITEM0(20)
   integer IFI,IDE,IC,I_ZONE,I_SECT
   integer retour
   INTEGER ILINE,NLINE,ICOLO,NCOLO
   REAL(DOUBLE),allocatable:: Q_XAJ(:,:)
   REAL(DOUBLE),allocatable:: X_XAJ(:)
   REAL(DOUBLE), dimension(NB_SECT) :: QINFLOW
   REAL(DOUBLE) XDIS
   real(DOUBLE) FROD,ZFIX,YFIX,DTPREC
   real(DOUBLE) SLD,QLD,SLF,QLF,C1
   ...
   save ITEM0,IDEB,IFIN,NBARAD,Q_XAJ,NLINE,X_XAJ,NCOLO
...
!
  ...
  JGNODE,JDNODE                  , &
      ...
      ! IF(.TRUE.) THEN
      !    PRINT*,'PRE - TIME =',TEMPS
      !    PRINT*,'PRE - UNODE=',UNODE(894)
      ! ENDIF

      if( Erreur%Numero /= 0 ) then
         Erreur%Message = 'Model initialization failed : check the vertical discretisation of the cross-sections'
         return
      endif
   ...
   elseif( PhaseSimulation == PHASE_TERMINAISON ) then
      ...
      if( associated(Sauve%H2OTB) )  deallocate( Sauve%H2OTB )
      if( associated(Sauve%H2OEB) )  deallocate( Sauve%H2OEB )
      if( associated(Sauve%H2OSB) )  deallocate( Sauve%H2OSB )
      if( associated(Sauve%H2OIC) )  deallocate( Sauve%H2OIC )
      if( associated(Sauve%H2OTC) )  deallocate( Sauve%H2OTC )
      if( associated(Sauve%H2OEC) )  deallocate( Sauve%H2OEC )
      if( associated(Sauve%H2OSC) )  deallocate( Sauve%H2OSC )
      if( associated(Sauve%H2OTBS) ) deallocate( Sauve%H2OTBS )
      if( associated(Sauve%H2OIBS) ) deallocate( Sauve%H2OIBS )
      if( associated(Sauve%SPREC) )  deallocate( Sauve%SPREC )
      if( associated(Sauve%QPREC) )  deallocate( Sauve%QPREC )
      return
   endif
   ...
   label_boucle_CL : do ICL = 1 , nb_ext
      ...
      FROD   = dabs( FROUD(IS) )
      ! modification maurice
      !      PRINT*,'ZFIX, QFIX, YFIX FROD =',ZFIX,QFIX,YFIX,FROD
      !
      !     TEST SUR LA HAUTEUR D'EAU EN REGIME TORRENTIEL
      !
      IF( ( IS ==1 ).and.( Frod >= 0.99D0 ) )  then
         If( YFIX <= 0.D0 ) then
            PRINT*,'IS, FROD, COTR(IS), TIME=',IS,FROD,COTR(IS),temps
            PRINT*,'ZFIX, QFIX, YFIX =',ZFIX,QFIX,YFIX
!            ---YFIX = 10.D0---
!            Erreur%Message = 'Negative water depth at the upstream node (supercritical)'
!            Erreur%Numero = 1
!            return
         endif
      endif
      ...
      if( ( ( AMONT .eqv. .True. ).and.( KHYLIM == CONDITION_TYPE_COTE_IMPOSE )).or.   &
         ...
         ! PRINT*,'TEMPS     = ',TEMPS
         ! IF((MOD(Temps,120.d0).EQ.0).AND.(TEMPS.GT.114000)) THEN
         !    PRINT*,'======'
         !    PRINT*,'TIME =',TEMPS,IS
         !    PRINT*,'UNODE=',UNODE(IS)
         ! ENDIF
         call LIMITE           ( &
              Y1(IS)           , &
              S1(IS)           , &
         ...
         ! IF((MOD(Temps,1200.d0).EQ.0).AND.(TEMPS.GT.86400)) PRINT*,'AFTER LIMITE Q1=',TEMPS,IS,Q1(IS)
      else
         ...
         !         DEBIT IMPOSE
            ...
            if( KHYLIM == CONDITION_TYPE_COTE_IMPOSE ) then
             ...
             SECTIONPLAN%DEBD , &
               ...
               ! IF((MOD(Temps,3600.d0).EQ.0).AND.(TEMPS.GT.86400)) PRINT*,'IN ELSE, Q1=',TEMPS,IS,Q1(IS)
            endif
         endif
      endif
...
!MS2018 problem evolution Courlis ==> solv by desactivating the option "calcOndesubmersion"?
   ...
   ! if(mod(temps,100.d0) == 0) print*,'time is now = ',Temps
   if(Temps == 0) then
      NLINE = 0
      call READ_Q_XAJ(nline,Q_XAJ)
      CALL READ_X_XAJ(NCOLO,X_XAJ)
      ! PRINT*,'NB_SECT=',Nb_sect
      ! PRINT*,'Q_XAJ === ',Q_XAJ(1,1),Q_XAJ(2,1),Q_XAJ(1,2)
      ! PRINT*,'X_XAJ === ',X_XAJ(1),X_XAJ(2)
   ENDIF
   DO IS =2, SIZE(QIN)
      QINFLOW(IS) = 0
   ENDDO
   DO IS =2, SIZE(QIN)
      DO ICOLO =1, NCOLO-1
         IF((X(IS)>=X_XAJ(ICOLO)).AND.(X(IS)<X_XAJ(ICOLO+1))) THEN
            ! PRINT*,'TEST HERE 11111'
            ! CASE WHERE Q_1 - Q_LAST-1
            XDIS = X_XAJ(ICOLO+1)-X_XAJ(ICOLO)
            DO ILINE = 1, NLINE-1
               IF(Temps>=Q_XAJ(ILINE,1).AND.(Temps<Q_XAJ(ILINE+1,1))) THEN
                  QINFLOW(IS) = (Q_XAJ(ILINE,ICOLO+1) + &
                  (Q_XAJ(ILINE+1,ICOLO+1)-Q_XAJ(ILINE,ICOLO+1)) * &
                  (Temps - Q_XAJ(ILINE,1)) / (Q_XAJ(ILINE+1,1)-Q_XAJ(ILINE,1))) / &
                  XDIS
                  CYCLE
               ENDIF
            ENDDO
         ...
         CYCLE
      ENDDO
      IF(X(IS)>=X_XAJ(NCOLO)) THEN
         ! CASE WHERE Q_LAST
         XDIS = X_XAJ(NCOLO) - X_XAJ(NCOLO-1)
         DO ILINE = 1, NLINE-1
            IF(Temps>=Q_XAJ(ILINE,1).AND.(Temps<Q_XAJ(ILINE+1,1))) THEN
               QINFLOW(IS) = (Q_XAJ(ILINE,NCOLO+1) + &
               (Q_XAJ(ILINE+1,NCOLO+1)-Q_XAJ(ILINE,NCOLO+1)) * &
               (Temps - Q_XAJ(ILINE,1)) / (Q_XAJ(ILINE+1,1)-Q_XAJ(ILINE,1))) / &
               XDIS
               CYCLE
            ENDIF
      ...
      ENDIF          
      ! PRINT*,'TEMPS = ',Temps
      ! PRINT*,'IS,QIN,QXAJ = ',IS,QIN(IS),QINFLOW(IS)
666      QIN(IS) = (QIN(IS)+QINFLOW(IS)) / ( X(IS) - X(IS-1) )
   ENDDO
   ! Modified by Maurice
   ! do IS = 2 , size(QIN)
   !    PRINT*,'TEMPS = ',Temps
   !    PRINT*,'IS,QIN,QXAJ = ',IS,QIN(IS),QINFLOW
   !    print*,NLINE
   !    QIN(IS) = (QIN(IS)+QINFLOW) / ( X(IS) - X(IS-1) )
   !    PRINT*,'IS,QIN = ',IS,QIN(IS)
   ! end do
   !
...
!MS2018 mise commentaire probleme evolution
      ...
      ! IF((MOD(Temps,1200.d0).EQ.0).AND.(TEMPS.GT.86400)) THEN
      !    PRINT*,'BEFORE RESOL'
      !    PRINT*,'UNODE=',UNODE(894)
      ! ENDIF
      call RESOL                   (  &
          SNODE                    , &
          QNODE                    , &
