...
In order to give all informations about the breach process, the user has to
...
\telkey{INITIAL LENGTHS OF BREACHES} (later lenghts has been replaced by widths).
If the initial widths are unknwown, \telemac{2D} will automatically compute the
...
and the distance betweeen the central points of the polyline defining the breach.
Indeed in \telemac{2D} we assume that the breaching process will start at the
