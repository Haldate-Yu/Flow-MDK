...
Parameterising the listing logical unit is necessary because it is not always 6
...
programs, like $T1$, $T2$, etc., in \telemac{2D} (note that they are $BIEF_OBJ$
structures, which brings some protection against misuses).  If it cannot be
...
THEN…$ are very risky if $X$ is the result of a computation. Allow some
tolerance, like: $IF(ABS(X).LT.1.D-10) THEN…$, especially if divisions are
involved.
